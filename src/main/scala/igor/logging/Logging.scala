...
trait Logging extends scalalogging.Logging {
  ...
  def alterMessages(logger: slf4j.Logger)(rewrite: => String => String): scalalogging.Logger = {
    new scalalogging.Logger {
      ...
      override def warn(message: String, cause: Throwable): Unit = logger.info(rewrite(message), cause)
      override def warn(message: String): Unit = logger.info(rewrite(message))
